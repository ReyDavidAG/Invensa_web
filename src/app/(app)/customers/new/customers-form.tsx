...
/* Hallmark · locked system applied (Taller) · src/app/(app)/customers/new/customers-form.tsx
 * New-customer form. RHF + zodResolver + useMutation (react-query).
 * Every control disables while the mutation is in flight.
 *
 * Single-column layout (no image dropzone here — that's products-only).
...
export function NewCustomerForm() {
  ...
  const isBusy = createCustomer.isPending;
      ...
      <fieldset disabled={isBusy} className="contents">
        <Card>
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <Field
              label="Notas"
              hint="Opcional. Anota preferencias, recordatorios o lo que ayude a la próxima venta."
              error={errors.notes?.message ?? fieldErrors?.notes?.[0]}
            >
              <textarea
                rows={3}
                autoComplete="off"
                placeholder="Prefiere aceite sintético, le fío los viernes…"
                {...register("notes")}
